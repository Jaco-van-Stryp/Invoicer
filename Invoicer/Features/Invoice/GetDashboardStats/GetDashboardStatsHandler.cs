    ...
    ) : IRequestHandler<GetDashboardStatsQuery, GetDashboardStatsResponse>
    {
        ...
        )
        {
            ...
            var statusSummary = new InvoiceStatusSummary(
                PaidCount: statusCounts.FirstOrDefault(s => s.Status == InvoiceStatus.Paid)?.Count ?? 0,
                PartialCount: statusCounts.FirstOrDefault(s => s.Status == InvoiceStatus.Partial)?.Count ?? 0,
                UnpaidCount: statusCounts.FirstOrDefault(s => s.Status == InvoiceStatus.Unpaid)?.Count ?? 0
            );
